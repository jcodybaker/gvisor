...
TEST_F(TuntapTest, SendUdpTriggersArpResolution) {
  ...
  inet_pton(AF_INET, kTapPeerIPAddr, &remote.sin_addr);
...
TEST_F(TuntapTest, TCPBlockingConnectFailsArpResolution) {
  ...
  inet_pton(AF_INET, kTapPeerIPAddr, &connectAddr.sin_addr.s_addr);
...
// fail on link address resolution failure to a routable, but non existent peer.
...
TEST_F(TuntapTest, WriteHangBug155928773) {
  ...
  remote.sin_family = AF_INET;
  remote.sin_port = htons(42);
  inet_pton(AF_INET, kTapIPAddr, &remote.sin_addr);
